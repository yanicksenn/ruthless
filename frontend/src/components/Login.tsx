...
import { ConfigPublic_AuthProvider } from '../api/config';

export const Login: React.FC = () => {
  const { user, loginWithToken, completeRegistration, authProvider, limits } = useAuth();
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="glass p-10 rounded-3xl w-full max-w-md card-shadow border border-white/5">
        ...
        <div className="space-y-8">
          {authProvider === ConfigPublic_AuthProvider.GOOGLE ? (
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  <span className="p-1 rounded bg-secondary/10"><UserPlus size={16} className="text-secondary" /></span>
                  Developer Login
                </h2>
                <p className="text-sm text-gray-400 leading-relaxed">
                  Backend is using fake auth. Enter a name or token to continue.
                </p>
              </div>
              <form onSubmit={async (e) => {
                ...
                <button
                  type="submit"
                  className="w-full bg-secondary hover:bg-secondary/80 text-background py-4 rounded-2xl font-black transition-all shadow-[0_0_20px_rgba(234,179,8,0.3)]"
                >
                  GO
                </button>
              </form>
